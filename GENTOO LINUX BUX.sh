...
USE="-systemd -gnome -kde -plasma -gtk-doc -oss -pipeware -dvd -dvdr -cdrom -cdr -bluray -doc -debug -test -selinux -audit -telemetry -ntfs -xfs -btrfs -xen"
LC_MESSAGES=C.utf8
...
if echo "app-shells/bash -net -pfs -mem-scramble -plugins -nls -doc -debug -test -selinux" > /etc/portage/package.use/bash; then
echo "PASSOU"
else
echo "FALHOU" && exit
fi;

...
sed -i 's/^.*CONFIG_DEBUG_PLIST.*$/CONFIG_DEBUG_PLIST=n/' .config;
sed -i 's/^.*CONFIG_DEBUG_SG.*$/CONFIG_DEBUG_SG=n/' .config;
sed -i 's/^.*CONFIG_DEBUG_NOTIFIERS.*$/CONFIG_DEBUG_NOTIFIERS=n/' .config;
sed -i 's/^.*CONFIG_DEBUG_MAPLE_TREE.*$/CONFIG_DEBUG_MAPLE_TREE=n/' .config;
sed -i 's/^.*CONFIG_KASAN.*$/CONFIG_KASAN=n/' .config;
sed -i 's/^.*CONFIG_HYPERV.*$/CONFIG_HYPERV=n/' .config;
sed -i 's/^.*CONFIG_AUDIT.*$/CONFIG_AUDIT=n/' .config;
sudo make -j$(nproc) && \
